...
function Item(props) {
    ...
    return (React.createElement("li", { className: ITEM_CLASS },
        ...
        React.createElement("button", { className: `${SHUTDOWN_BUTTON_CLASS} jp-mod-styled`, onClick: () => props.bind(model) }, "BIND\u00A0DATA")));
}
...
function RunningSessionsComponent({ session, manager, sessionOpenRequested }) {
...
export class RunningSessions extends ReactWidget {
    ...
    render() {
        return (React.createElement(RunningSessionsComponent, { session: this._session, manager: this.options.manager, sessionOpenRequested: this._sessionOpenRequested }));
    }
    /**
